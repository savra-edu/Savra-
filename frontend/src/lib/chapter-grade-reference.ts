 ...
 * Used to filter chapters by subject + grade in quiz, question paper, and lesson plan forms.
 *
 * Keys: subject code (matches backend Subject.code)
...
/** Subject code to API subject name mapping (for display) */
...
/**
 * Get chapter names for a subject and grade.
 * Returns empty array if no mapping exists (fallback to API-only filter).
 */
export function getChaptersForSubjectGrade(subjectCode: string, grade: number): string[] {
  const bySubject = CHAPTER_GRADE_REFERENCE[subjectCode];
  if (!bySubject) return [];
  const chapters = bySubject[grade];
  return chapters ?? [];
}
